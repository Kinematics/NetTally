...
namespace NetTally.Forums
{
    ...
    public class Post : IComparable<Post>, IEquatable<Post>, IComparable
    {
        ...
        private IReadOnlyList<VoteLine> GetPostAnalysisResults(string text)
        {
            ...
            bool IsTallyPost(string innerText)
            {
                // If the post contains the string "#####" at the start of the line for part of its text,
                // it's a tally post.
                string cleanText = VoteLineParser.StripBBCode(innerText.AsSpan());
            ...
            IEnumerable<VoteLine> GetVoteLines(List<string> postTextLines)
            {
                bool foundFirst = false;

                foreach (var line in postTextLines)
                {
                    var voteLine = VoteLineParser.ParseLine(line.AsSpan());
            ...
            List<VoteLine> GetNominationVoteLines(List<string> postTextLines)
            {
                List<VoteLine> resultLines = new List<VoteLine>();
