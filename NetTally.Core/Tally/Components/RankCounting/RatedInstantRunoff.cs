...
/// </summary>
public class RatedInstantRunoff : IRankVoteCounter
{
        ...
        GetWinningVote(VoteStorage votes)
    {
        ...
        VoteStorageEntryF winner =
            GetOptionWithHigherPrefCount(options[0].option, options[1].option);

        return (winner, winner.Key == options[0].option.Key ?
                        options[0].score :
                        options[1].score);
    }
    ...
    private static List<(VoteStorageEntryF option, double score)>
