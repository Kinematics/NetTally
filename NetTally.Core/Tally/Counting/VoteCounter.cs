...
namespace NetTally.VoteCounting
{
    public class VoteCounter : IVoteCounter
    {
        ...
        {
            if (ReferencePlans.TryGetValue(planOrigin, out VoteLineBlock value))
            {
                return value;
            }
        ...
        public bool Join(List<Origin> voters, Origin voterToJoin)
        {
            ...
            bool JoinImpl(Origin joiningVoter, Origin targetVoter)
            {
                var source = GetVotesBy(joiningVoter);
                var dest = GetVotesBy(targetVoter);
                ...
                {
                    if (!VoteStorage.DoesVoterSupportVote(targetVoter, vote))
                    {
