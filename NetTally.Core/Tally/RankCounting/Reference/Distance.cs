...
public class Distance : IRankVoteCounter
{
        ...
        CountVotesForTask(VoteStorage taskVotes)
    {
        ...
        List<((int rank, double rankScore) ranking, VoteStorageEntryF vote)> winningChoices =
    ...
    private static int[,] GetStrongestPaths(int[,] pairwiseData, int choicesCount)
    {
        ...
        int bytesInArray = data.Length * sizeof(int);
        ...
        GetResultsInOrder(int[,] winningPaths, List<VoteBlock> listOfChoices, VoteStorage taskVotes)
    {
        ...
        List<((int rank, double rankScore) ranking, VoteStorageEntryF vote)> results
            = [];

        for (int i = 0; i < orderPaths.Count; i++)
        {
            var entry = ((i + 1, (double)orderPaths[i].Count / orderPaths[i].Sum),
                new VoteStorageEntryF(orderPaths[i].Choice, taskVotes[orderPaths[i].Choice]));
            results.Add(entry);
        }
