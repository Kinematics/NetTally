...
public class ThreadInfoTests
{
    ...
    [TestMethod]
    public void No_ThreadInfo()
    {
        var threadInfo = ThreadInfo.None;
        Assert.AreEqual(string.Empty, threadInfo.Title);
        Assert.AreEqual(Author.None, threadInfo.Author);
        Assert.AreEqual(ThreadRange.None, threadInfo.ThreadRange);
    }

    [TestMethod]
    public void Construct_NoRange()
    {
        string title = "";
        Author? author = null;

        var threadInfo = ThreadInfo.Create(title, author, null);

        Assert.IsNull(threadInfo);
    }
