...
namespace NetTally
{
    public static class VoteString
    {
        ...
        static readonly Regex voteFullRegex = new Regex(@"^(?<prefix>-*)\s*\[\s*(?<marker>[xX+✓✔1-9])\s*\]\s*(\[\s*(?<task>[^]]*?)\s*\])?\s*(?<content>.*)", RegexOptions.Singleline);
        ...
        static readonly Regex taskRegex = new Regex(@"^(?<pre>.*?\[[xX+✓✔1-9]\])\s*(\[\s*(?<task>[^]]*?)\s*\])?\s*(?<remainder>.+)", RegexOptions.Singleline);
